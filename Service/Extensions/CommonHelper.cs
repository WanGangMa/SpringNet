...
namespace Service
{
    ...
    {
        public ICodeAreaManage CodeAreaManage = DependencyResolver.Current.GetService<ICodeAreaManage>();

        public ICodeManage CodeManage = DependencyResolver.Current.GetService<ICodeManage>();

        public IUserManage UserManage = DependencyResolver.Current.GetService<IUserManage>();

        public IProjectFilesManage ProjectFilesManage = DependencyResolver.Current.GetService<IProjectFilesManage>();

        public IProjectStageManage ProjectStageManage = DependencyResolver.Current.GetService<IProjectStageManage>();

        public IContentManage ContentManage = DependencyResolver.Current.GetService<IContentManage>();
        ...
        public MvcHtmlString GetModuleMenu(SYS_MODULE module, List<SYS_MODULE> moduleList)
        {
            ...
            List<SYS_MODULE> list = (from p in moduleList.FindAll((SYS_MODULE p) => p.PARENTID == module.ID && p.LEVELS == 1)
                                     orderby p.SHOWORDER
        ...
        {
            List<SYS_MODULE> list = (from p in moduleList.FindAll((SYS_MODULE p) => p.PARENTID == module.ID)
                                     orderby p.SHOWORDER
                                     select p).ToList<SYS_MODULE>();
            if (list != null && list.Count > 0)
            {
        ...
        public string GetCodeAreaName(string codearealist)
        {
            ...
            List<string> list = (from p in codearealist.Trim(new char[]
            {
                ','
            }).Split(new string[]
            {
        ...
        {
            return (from p in this.ProjectFilesManage.LoadAll((PRO_PROJECT_FILES p) => p.DocStyle == "projectstage" && p.Fk_ForeignId == stageId && p.FK_UserId == userId)
