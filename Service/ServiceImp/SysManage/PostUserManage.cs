...
namespace Service.ServiceImp
{
    ...
    public class PostUserManage : RepositoryBase<Domain.SYS_POST_USER>, IService.IPostUserManage
    {
        ...
        {
            return this.LoadAll(p => userId.Contains(p.FK_USERID.ToString())).Select(p => p.SYS_POST).ToList();
        }
        ...
        public bool SavePostUser(int userId, string postId)
        {
            try
            {
                ...
                {
                    //存在之后再对比是否一致 
                    var oldCount = this.LoadAll(p => p.FK_USERID == userId).Select(p => p.FK_POSTID).ToList().Cast<int>().ToList();
        ...
        {
            //岗位部门关系ID集合
            string post_departmentid = collection.Select(p => p.FK_POSTID).Aggregate(string.Empty, (current, t) => current + "'" + t + "',").TrimEnd(',');
            try
            {
