    ...
    public class PermissionController : BaseController
    {
        ...
        public ActionResult GetTree()
        {
            ...
            catch (Exception e)
            {
                json.Status = "n";
                json.Msg = "服务器忙，请稍后再试！";
                WriteLog(enumOperator.Select, "权限管理，获取模块树：", e);
            }
            return Json(json);
        }
        ...
        public ActionResult SaveAllocation()
        {
            ...
            try
            {
                if (tp == "user")
                {
                    if (!this.UserPermissionManage.SetUserPermission(int.Parse(id), perid)) { json.Msg = "保存失败"; WriteLog(enumOperator.Allocation, "设置用户权限，结果：" + json.Msg, enumLog4net.ERROR); return Json(json); }
                }
                else if (tp == "role")
                {
                    if (!this.RolePermissionManage.SetRolePermission(int.Parse(id), perid, sysid)) { json.Msg = "保存失败"; WriteLog(enumOperator.Allocation, "设置角色权限，结果：" + json.Msg, enumLog4net.ERROR); return Json(json); }
                }
