    ...
    public class ChatHub:Hub
    {
        ...
        public void Send(string message, string groupId)
        {
            try
            {
                ...
                {
                    //保存消息
                    ChatMessageManage.Save(new Domain.SYS_CHATMESSAGE() { FromUser = UserOnline.FK_UserId, MessageType = ClsDic.DicMessageType["广播"], MessageContent = message, MessageDate = DateTime.Now, MessageIP = Utils.GetIP() });
                    //返回消息实体
                ...
                {
                    //保存消息
                    ChatMessageManage.Save(new Domain.SYS_CHATMESSAGE() { FromUser = UserOnline.FK_UserId, MessageType = ClsDic.DicMessageType["群组"], MessageContent = message, MessageDate = DateTime.Now, MessageIP = Utils.GetIP(), ToGroup = groupId });
                    //返回消息实体
        ...
        public void SendSingle(string clientId, string message)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        //保存消息
                        ChatMessageManage.Save(new Domain.SYS_CHATMESSAGE() { FromUser = UserOnline.FK_UserId, MessageType = ClsDic.DicMessageType["私聊"], MessageContent = message, MessageDate = DateTime.Now, MessageIP = Utils.GetIP(), ToGroup = UserOnline.SYS_USER.ID.ToString() });
                        //返回消息实体
